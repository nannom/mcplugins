execute as @a run function skul:file2
...
#공격시
execute as @e[tag=click] on attacker at @e[tag=attack] if score @s attack_cooldown matches ..0 run function skul:file4
...
execute as @e[tag=click] on target run function skul:file6
execute as @e[tag=click] if data entity @s interaction run data remove entity @s interaction

#대쉬
execute as @a at @s if score @s dash matches 1.. run function skul:file7

#대쉬 종료
execute as @a at @s if score @s dash matches 8.. run function skul:file8

#점프 이팩트
execute as @a at @s if score @s jump matches 1.. run function skul:file9
...
execute if data entity @e[tag=test,limit=1] {ArmorItems:[{id:"minecraft:leather_horse_armor"}]} as @a run function skul:file10

#아이템 버림 이벤트
execute as @e[type=item] at @s run function skul:file11

execute as @e[type=item,tag=!canpickup] run data merge entity @s {PickupDelay:0s,Age:0s}
execute as @a if data entity @s Inventory[{Slot:-106b}] run function skul:file14